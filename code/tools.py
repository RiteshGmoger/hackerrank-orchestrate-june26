...
def retrieve_docs(query: str, domain: str) -> str:
    ...
    try:
        rephrase_response = _call_with_retry(lambda: client.messages.create(
            ...
            max_tokens = 100,
            temperature = 0.0,
            messages = [{"role": "user", "content": f"""Produce 2 alternative phrasings of this query using different words, same meaning.Query: {query}Output ONLY valid JSON: {{"q1": "...", "q2": "..."}}"""}]
            ))
        alt = json.loads(rephrase_response.content[0].text)
        queries = [query, alt.get("q1", query), alt.get("q2", query)]
    except Exception:
        queries = [query]  # single query if rephrasing fails
...
def check_escalation(ticket_text: str, retrieved_chunks: str) -> str:
    """
        ...
        false escalations are cheap, hallucinated policies are expensive
    ...
    try:
        response = _call_with_retry(lambda: client.messages.create(
            ...
            Available documentation: {retrieved_chunks}
